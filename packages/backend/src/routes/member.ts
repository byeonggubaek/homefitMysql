...
import {
  addMemberPlan, deleteMemberPlan, getMember, getMemberships, getMonthStatus,
  insertMember, login, MemberPlans, getRecentWorkoutRecords, checkPwMatch,
  updateMemberInfo, select, // 💡 select 추가
  getWorkoutDetails
} from '../db.js';
...
// ---------------------------------------------------------
// 1. [해결] 'genAI' 이름을 찾을 수 없습니다 에러 해결
// ---------------------------------------------------------
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || "");
const memberRouter = express.Router();

// --- 헬퍼 함수: DB에서 기존 AI 리포트가 있는지 확인 ---
const getAiReportByWorId = async (wor_id: number | string) => {
  const query = `SELECT * FROM T_AI_REPORT WHERE WOR_ID = ?`;
  const results = await select(query, [wor_id]);
...
memberRouter.post("/login", async (req, res) => {
  ...
  try {
    const { mem_id_act, mem_password } = req.body;
    if (!mem_id_act) return res.status(401).json({ success: false, error: '회원 ID가 필요합니다.' });
    if (!mem_password) return res.status(401).json({ success: false, error: '회원 비밀번호가 필요합니다.' });
  ...
  try {
    ...
    if (req.session) {
      req.session.destroy((err) => {
        if (err) console.error('세션 제거 오류:', err);
      });
    }
...
memberRouter.get('/getMember', async (req, res) => {
  ...
  try {
    ...
    if (!mem_id) return res.status(400).json({ success: false, error: '회원 ID가 필요합니다.' });
...
memberRouter.post('/insertMember', async (req, res) => {
  ...
  try {
    ...
    const memberData: T_MEMBER = {
      MEM_ID_VIEW: '', MEM_NAME: mem_name, MEM_NICKNAME: mem_nickname ?? null,
      MEM_PASSWORD: mem_password, MEM_IMG: mem_img ?? null, MEM_PNUMBER: mem_pnumber ?? null,
      MEM_EMAIL: mem_email ?? null, MEM_SEX: mem_sex, MEM_AGE: Number(mem_age) || 0,
      MEM_POINT: 0, MEM_EXP_POINT: 0, MEM_LVL: 0, MEM_STREAK: 0, MES_ID: Number(mes_id),
    };
...
memberRouter.post('/insertMemberPlan', async (req, res) => {
  ...
  try {
    ...
    await addMemberPlan({
      MEM_ID: Number(MEM_ID), WOO_ID: WOO_ID, MEP_DATE: MEP_DATE,
      MEP_TARGET_REPS: Number(MEP_TARGET_REPS), MEP_TARGET_SETS: Number(MEP_TARGET_SETS), MEP_UNIT: MEP_UNIT
    });
    await Logger.logApiSuccess(apiLogEntry);
...
`;
    ...
    await select(insertQuery, [
      FINAL_WOR_ID,
      finalSummary,
      JSON.stringify(finalRecs), // 배열은 꼭 문자열로!
      finalIntensity,
      finalRank
    ]);
