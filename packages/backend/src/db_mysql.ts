...
// `);
// }
// async function _getSubMenus(title: string = ''): Promise<any[]> {
//   return select(`
// SELECT  CONCAT(nav_id, '-', nas_id) AS id,
//         nas_name title,
//         nas_href href,
//         nas_desc description
// FROM    t_nav_sub_item
// WHERE   nas_name LIKE CONCAT('%', ?, '%')
// ORDER BY nav_id, nas_id
// `, [title]);
// }
// async function _getRawCurMenuPos(page: string = ''): Promise<any[]> {
//   return select(`
// SELECT  JSON_OBJECT(
//          'id', a.nas_id,
//          'parent_title', b.nav_name,
//          'title', a.nas_name,
//          'siblings',
//            COALESCE(
//              (
...
//                WHERE s.nav_id = a.nav_id   -- 현재 메뉴 자식 조회
//              ),
//              JSON_ARRAY()
//            )
//        ) AS result
...
//         nas_desc description
// FROM    t_nav_sub_item
// WHERE   nas_name LIKE CONCAT('%', UPPER(?), '%') OR nas_desc LIKE CONCAT('%', UPPER(?), '%')
// ORDER BY nav_id, nas_id
// `, [cleanKey, cleanKey]);
// }
...
//   return select(`
// SELECT  Lower(col_id) AS id,  -- 중요함: 칼럼명을 소문자로 변환하여 반환
//         col_name title,
//         col_type type,
//         col_width width,
//         col_sum summary
...
//   const subMenus = await _getSubMenus(title);
...
//   const result = await _getRawCurMenuPos(page);
