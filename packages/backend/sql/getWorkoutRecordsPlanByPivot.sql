DROP PROCEDURE IF EXISTS getTSDSDFsdfByPivot;
DELIMITER $$

CREATE PROCEDURE getTSDSDFsdfByPivot(
    IN  p_mem_id INT,
    IN  p_from_dt DATE,
    IN  p_to_dt DATE,
    OUT VSQL LONGTEXT,     -- 생성된 동적 쿼리 (대문자)
...
BEGIN
    ...
    -- JSON 및 그룹 합계 사이즈 제한 해제
    SET SESSION group_concat_max_len = 4294967295; 

    -- 1) T_WORKOUT 기준으로 동적 피벗 컬럼 생성 (실적 WOO_ID, 계획 WOO_ID_P)
    SELECT 
        GROUP_CONCAT(
            CONCAT(
    ...
    -- 2) 동적 쿼리 작성
    -- RECURSIVE CTE를 사용하여 기간 내 모든 날짜를 생성 (운동 안 한 날 포함)
    SET v_sql = CONCAT(
        ...
        'INTO @DATA_TMP FROM ( ',
        '    SELECT DATE_FORMAT(D.DT, "%d") AS WO_DT, ', 
        '           ', v_cols_expr,
        '    FROM DATE_RANGE D ',
    ...
    -- 파라미터 순서: DATE_RANGE(?), DATE_RANGE(?), BASE_DATA_PLAN(?,?,?), BASE_DATA_ACT(?,?,?)
